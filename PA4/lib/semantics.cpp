...
char* get_expression_type(ASTNode* node) {
	if (!node) return sem_strdup("void");

	switch (node->type) {
	case NODE_LITERAL:
		if (strcmp(node->data, "true") == 0 || strcmp(node->data, "false") == 0)
			return sem_strdup("bool");
		if (strchr(node->data, '.')) return sem_strdup("float");
	...
	case NODE_FUNC_CALL: {
		Symbol* sym = lookup_symbol(node->data);
		if (sym) return sem_strdup(sym->type);
...
int is_compatible(char* targetType, char* expressionType) {
	if (strcmp(expressionType, "unknown") == 0) return 1;

	if (strcmp(targetType, expressionType) == 0) return 1;
